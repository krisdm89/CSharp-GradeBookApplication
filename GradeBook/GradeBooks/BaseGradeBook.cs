...
namespace GradeBook.GradeBooks
{
    public abstract class BaseGradeBook
    {
        ...
        {
            bool addPoint = (IsWeighted && (studentType == StudentType.DualEnrolled || studentType == StudentType.Honors));
            switch (letterGrade)
            {
                case 'A':
                    if (addPoint) { return 5; } else { return 4; };
                case 'B':
                    if (addPoint) { return 4; } else { return 3; };
                case 'C':
                    if (addPoint) { return 3; } else { return 2; };
                case 'D':
                    if (addPoint) { return 2; } else { return 1; };
                case 'F':
                    if (addPoint) { return 1; } else { return 0; };
            }
            return 0;
        }
